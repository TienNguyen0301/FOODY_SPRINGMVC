    ...
    <nav class="navbar navbar-expand-lg navbar-light py-lg-0 px-lg-5 wow fadeIn" data-wow-delay="0.1s">
        ...
        <div class="collapse navbar-collapse" id="navbarCollapse">
            <div class="navbar-nav ms-auto p-4 p-lg-0">
                ...
                <c:if test="${pageContext.request.userPrincipal.name != null}">
                    <c:set var="role" value="ROLE_STORE"/>
                    <c:if test="${currentUser.userRole.equals(role)}">
                        <a href="<c:url value="/store/registerFood" />" class="nav-item nav-link text-success">Đăng món ăn</a> 
                    </c:if>
                    <c:set var="roleAd" value="ROLE_ADMIN"/>
                    <c:if test="${currentUser.userRole.equals(roleAd)}">
